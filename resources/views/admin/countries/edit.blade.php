...
<section class="m-8">
    ...
    <div class="container md:w-3/4 mx-auto px-5 mt-16">
        ...
        @endif
        <form action="{{route('countries.update', $country)}}" method='post' class="flex flex-col w-full" onsubmit="return validate(event)">
            @csrf
            @method('PATCH')
            <label for="" class='mt-8'>Country Name</label>
            <input type="text" id='name' name='name' class="input" placeholder="Pakistan" value="{{$country->name}}">
